...
startGame :: Deck -> Game
startGame
  deck@(p1 : d1 : p2 : d2 : tail) = Game (Player [p1, p2]) (Player [d1, d2]) PlayerTurn tail
...
hit :: Game -> Game
hit
  game@(Game player@(Player playerHand) casino@(Player casinoHand) gStatus deck@(casinoHead : casinoTail))
    | gStatus == PlayerTurn =
